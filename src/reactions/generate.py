...
def generate_data(
    ...
    p0 = Parallel(n_jobs=num_cores)(delayed(src.reactions.fingerprint.calc_fp_individual)(i, radius, nBits) for i in inputs)

    inputs = tqdm(df['product_1'])
    p1 = Parallel(n_jobs=num_cores)(delayed(src.reactions.fingerprint.calc_fp_individual)(i, radius, nBits) for i in inputs)

    inputs = tqdm(df['product_2'])
    p2 = Parallel(n_jobs=num_cores)(delayed(src.reactions.fingerprint.calc_fp_individual)(i, radius, nBits) for i in inputs)

    inputs = tqdm(df['product_3'])
    p3 = Parallel(n_jobs=num_cores)(delayed(src.reactions.fingerprint.calc_fp_individual)(i, radius, nBits) for i in inputs)
