...
print("[pred] true", (pd.Series(train_catalyst.numpy().argmax(axis=1)).value_counts() / train_catalyst.shape[0]).iloc[:5], sep="\n")
print("[pred] pred", (pd.Series(cat_pred.numpy().argmax(axis=1)).value_counts() / cat_pred.shape[0]).iloc[:5], sep="\n")

# %%

h = model.fit(
    x=x_train_data, y=y_train_data, 
    epochs=1, verbose=1, batch_size=5096,
