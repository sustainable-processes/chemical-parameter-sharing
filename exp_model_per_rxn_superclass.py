...
train_super_class, val_super_class = df['rxn_super_class'].iloc[train_idx].reset_index(drop=True), df['rxn_super_class'].iloc[val_idx].reset_index(drop=True)
...
for sc in super_classes:
    ...
    for t in targets:
        ...
        untrained_predictions[sc]['val'][t] = {"true": val_data[t], "pred": m.forward_dict(data=val_data)[t]}
    ...
    for t in targets:
        ...
        predictions[sc]['val'][t] = {"true": val_data[t], "pred": m.forward_dict(data=val_data)[t]}
...
train_acc = src.learn.metrics.get_topk_acc(
    pred=torch.cat([untrained_predictions[p]["train"]["catalyst"]['pred'] for p in untrained_predictions]), 
    true=torch.cat([untrained_predictions[p]["train"]["catalyst"]['true'] for p in untrained_predictions]),
    k=[1,5],
)
val_acc = src.learn.metrics.get_topk_acc(
    pred=torch.cat([untrained_predictions[p]["val"]["catalyst"]['pred'] for p in untrained_predictions]), 
    true=torch.cat([untrained_predictions[p]["val"]["catalyst"]['true'] for p in untrained_predictions]),
    k=[1,5],
)
